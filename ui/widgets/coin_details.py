...
# (network/threading removed — stats arrive from core via UIAPI)
import logging
...
class CoinDetailsPanel(ttk.Frame):
    ...
    def on_stats(self, symbol: str, high: float, low: float, vol: float) -> None:
        """
        UI-safe: 24h stats приходят из core (через UIAPI/snapshot), UI сеть не трогает.
        """
        if symbol != self._symval:
            return
        self._set_stats(high, low, vol)
